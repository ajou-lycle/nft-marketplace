...
export const mint = async (eth) => {
  return new Promise(async (resolve, reject) => {
    ...
    await axios('http://3.38.210.200:3000/nonce')
      .then(async (response) => {
        ...
        const account = await eth.web3.eth.accounts.privateKeyToAccount('e7a36cc7c2c6aa2c49be21b5a8516982fce19e4337f3c0b56f73a401800eb1cf');
        const signed = await eth.web3.eth.accounts.signTransaction(tx, account.privateKey);
        let result = await eth.web3.eth.sendSignedTransaction(signed.rawTransaction);
      });
